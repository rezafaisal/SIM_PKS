...
const CollaborationTable = ({ data, levelStatus }: any) => {
      ...
      const fixData =
        levelStatus.name === "Admin"
          ...
          : data.filter((item: any) =>
              item.prodi.some((prodi: any) => prodi.id === levelStatus.id)
            );

      setDatas(
        fixData.map((item: any, index: number) => {
          return {
            ...item,
            no: index + 1,
            mitra_penandatangan: {
  ...
  return (
    <div className="bg-white px-5 pt-4 pb-6 rounded-xl shadow">
      <div className=" text-black text-[22px] font-semibold ">
        Table Perjanjian Kerjasama{" "}
        {levelStatus.name != "Admin" ? (
          <span className="text-orange-600 capitalize">{levelStatus.name}</span>
        ) : (
          ""
        )}
      </div>
      ...
      <Table2
        ...
        renderItem={(collaboration) => (
          <tr key={collaboration.id} className="">
            ...
            <td
              key={columns[2].id}
              className="py-5 px-4 font-bold text-sm text-slate-600 min-w-[20rem] !whitespace-normal"
            >
