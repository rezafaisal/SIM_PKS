...
import { CollaborationMakerPDF } from "./CollaborationMakerPDF";
...
const CollaborationDetail = ({
  ...
  const { mutateAsync, isLoading } = useDeleteCollaboration({
    config: {
      onSuccess() {
        closeAllModals();
      },
  ...
  function handleRemove(e: any, id: number) {
    e.preventDefault();
    openConfirmModal({
      title: "Hapus Kerjasama",
      children: (
        <Text size="sm">Apakah anda yakin untuk menghapus kerjasama ini?</Text>
      ),
      centered: true,
  ...
  return (
    <section className="relative px-3 pb-5">
      ...
      <div className="flex flex-wrap items-center justify-center md:justify-end gap-4 mt-10">
        {levelStatus == "Admin" ? (
          <a href="/">
            <Button
              onClick={(e) => handleRemove(e, collaboration.id)}
              type="button"
              loading={isLoading}
        ...
        <a href={`/kerjasama/edit/${collaboration.id}`}>
