...
import { CreateProdiDTO, useCreateProdi } from "../api";
...
const ProdiCreateForm: React.FC<Props> = ({ onSuccess, dataFaculty }) => {
  const form = useForm<CreateProdiDTO["data"]>({
    initialValues: {
      name: "",
      id_faculty: null,
    },
  ...
  const { mutateAsync, isLoading } = useCreateProdi({
    config: {
      onError({ response }) {
...
export default ProdiCreateForm;
