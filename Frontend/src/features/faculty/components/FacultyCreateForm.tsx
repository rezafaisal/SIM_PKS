...
import { CreateFacultyDTO, useCreateFaculty } from "../api";

interface Props {
  onSuccess: VoidFunction;
}

const FacultyCreateForm: React.FC<Props> = ({ onSuccess }) => {
  const form = useForm<CreateFacultyDTO["data"]>({
    initialValues: {
      name: "",
    },
  });

  const { mutateAsync, isLoading } = useCreateFaculty({
    config: {
      onError({ response }) {
